...
for region in ${regions}; do
  ...
  version=$(aws --region ${region} lambda publish-layer-version --layer-name php71 --zip-file fileb://php71.zip --output text --query LayerVersionArn)
  echo "Published Lambda Layer php71 in region ${region}, Layer ARN: ${version}"
